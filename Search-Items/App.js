...
class App extends Component {
  ...
  constructor() {
    ...
    this.state = {
      ...
      defaultItems: ['Potato', 'Tomato', 'Pumpkin'],
      buttonFlag: true
    }
  }
  ...
  searchValueChanged() {
    if (this.searchItem.value.length === 0) {
      this.setState({
  ...
  search() {
    var searchedItems = []
    var tempItems = []
    Object.assign(tempItems, this.state.items)

    searchedItems = tempItems.filter(s => s.indexOf(this.searchItem.value) !== -1)
    this.setState({
      items: searchedItems
    })
  }
    ...
    return (
      <div className="container">
        ...
        <div className="input-group mb-3">
          <input type="text" className="form-control" placeholder="enter item to be searched..."
            ref={(input) => { this.searchItem = input }} onInput={() => this.searchValueChanged()} />
          <button className="btn btn-outline-primary material-icons" onClick={() => this.search()}>search</button>
        </div>
